 ...
 * 3. rendering mold (@see mold/<%=name.toLowerCase()%>.js )
 ...
 */
(() => {
<%=name.toLowerCase()%>.<%=name%> = zk.$extends(zul.Widget, {
	...
	$define: {
		 ...
		 *
		 */
		text() { //this function will be called after setText() .
			if (this.desktop) {
				//updated UI here.
			}
		}
	...
	*/
	bind_() {
		 ...
		 */
		this.$supers(<%=name.toLowerCase()%>.<%=name%>, 'bind_', arguments);
	...
	*/
	unbind_() {
		...
		*/
		this.$supers(<%=name.toLowerCase()%>.<%=name%>, 'unbind_', arguments);
	},
	/*
	...
	doClick_(evt: zk.Event) {
		this.fire('onFoo', {foo: 'myData', stamp: this._getStamp()});
		this.$supers('doClick_', arguments); //the super doClick_ should be called
	},
	/*
		A sample of private method.
	*/
	_getStamp(): number {
		return Date.now();
	}
